...
async def on_message(socket, data):
	...
	try:
		...
		if data["type"] == "PLAY_CLIP":
			...
			thread = threading.Thread(target=play_sound, args=((data["fp"], cable_output, socket, asyncio.get_event_loop())))
			thread.start()
...
async def get_cable_output(socket):
	...
	for device in devices:
		if "(vb-audio virtual" in device["name"].lower() and device["hostapi"] == 0:
